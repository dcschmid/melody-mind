...
### Aesthetic Direction
Inspired by editorial music publications (NPR Music, Pitchfork, The Guardian's music coverage) — warm paper-like backgrounds, confident typography, generous whitespace, restrained decoration. Content-first layouts with clear hierarchy. The site should feel like a beautifully typeset magazine, not a dashboard or admin panel.

**Theme**: Light mode as the primary experience (warm, paper-like canvas). Dark mode as an alternative for nighttime reading. The codebase already supports `data-theme` switching, so both should be first-class citizens.

**Anti-references**: AI slop aesthetics (neon gradients on dark, cyan glow, purple-to-blue gradients), generic card grids with identical structure, side-stripe borders on cards, gradient text, glassmorphism, bounce animations.
...
3. **Purposeful color** — The warm amber/brown palette is already distinctive. Lean into it. Use color sparingly and meaningfully: accents work because they're rare. Avoid decorative gradients or glowing effects.
