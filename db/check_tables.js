...
async function checkTables() {
  try {
    ...
    const oauthTables = result.rows.filter(
      (row) =>
        row.name.includes("oauth") || row.name.includes("provider") || row.name.includes("session")
    );
    ...
    usersInfo.rows.forEach((row) => {
      console.log(
        `- ${row.name}: ${row.type} (${row.dflt_value ? `default: ${row.dflt_value}` : "no default"})`
      );
    });
  } catch (error) {
