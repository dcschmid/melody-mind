---
format: artist-portrait
title: Heavy Kept Changing Shape
dek: Boris made heaviness a question of time, pressure, repetition, and form—then refused to give the same answer twice.
seoDescription: How Boris reshaped heavy music through Absolutego, Amplifier Worship, Flood, Heavy Rocks, Feedbacker, and Pink.
publishedAt: 2026-08-07T14:00:00Z
topics:
  - Boris
  - Japanese experimental rock
  - Drone music
  - Album form
byline: MelodyMind Editorial
draft: false
hero:
  id: boris-trio-2007
  image: ../../assets/heavy-kept-changing-shape-1.jpg
  alt: Boris performing as a trio on a dark stage, surrounded by amplifiers and warm red light.
  caption: "Boris in 2007: Takeshi, Wata, and Atsuo working inside an amplifier setup that is part instrument, part architecture."
  creator: Andy Newcome
  sourceName: Wikimedia Commons
  sourceUrl: https://commons.wikimedia.org/wiki/File%3ABoris_(band).jpg
  license: CC BY-SA 2.0
  licenseUrl: https://creativecommons.org/licenses/by-sa/2.0
  alterations: Resized and converted to optimized JPEG; Astro creates responsive derivatives
figures:
  - id: boris-profile
    image: ../../assets/heavy-kept-changing-shape-2.jpg
    alt: A side view of Boris performing through haze, with a large gong and stacked amplifiers visible onstage.
    caption: "The band's stage can hold several kinds of scale at once: a human figure, an amplifier wall, and the circular field of Atsuo's gong."
    creator: Romoromor
    sourceName: Wikimedia Commons
    sourceUrl: https://commons.wikimedia.org/wiki/File%3ABoris-Profile.jpg
    license: CC BY-SA 4.0
    licenseUrl: https://creativecommons.org/licenses/by-sa/4.0
    alterations: Resized and converted to optimized JPEG; Astro creates responsive derivatives
  - id: wata-guitar
    image: ../../assets/heavy-kept-changing-shape-3.jpg
    alt: Wata of Boris playing electric guitar onstage beneath blue light, with amplifiers behind her.
    caption: Wata's sustained guitar tone can carry melody, grain, and physical pressure without separating those roles.
    creator: Tim Bray
    sourceName: Wikimedia Commons
    sourceUrl: https://commons.wikimedia.org/wiki/File%3AWata,_guitarist_and_singer_with_Boris.jpg
    license: CC BY-SA 3.0
    licenseUrl: https://creativecommons.org/licenses/by-sa/3.0
    alterations: Resized and converted to optimized JPEG; Astro creates responsive derivatives
  - id: atsuo-drums
    image: ../../assets/heavy-kept-changing-shape-4.jpg
    alt: Atsuo of Boris leaning over his drum kit during a live performance under blue stage lighting.
    caption: "Atsuo at the kit: Boris's long forms depend on attack and pacing as much as on sustained guitar sound."
    creator: Tim Bray
    sourceName: Wikimedia Commons
    sourceUrl: https://commons.wikimedia.org/wiki/File%3AAtsuo,_drummer_and_singer_with_Boris.jpg
    license: CC BY-SA 3.0
    licenseUrl: https://creativecommons.org/licenses/by-sa/3.0
    alterations: Resized and converted to optimized JPEG; Astro creates responsive derivatives
  - id: boris-australia-2012
    image: ../../assets/heavy-kept-changing-shape-5.jpg
    alt: Boris performing live in Australia in 2012, with guitar, drums, smoke, and amber stage lights.
    caption: Boris in Australia in 2012, after the name Heavy Rocks had already been used for two markedly different albums.
    creator: Gwendolyn Lee
    sourceName: Wikimedia Commons
    sourceUrl: https://commons.wikimedia.org/wiki/File%3ABoris_2012_au.jpg
    license: CC BY-SA 2.0
    licenseUrl: https://creativecommons.org/licenses/by-sa/2.0
    alterations: Resized and converted to optimized JPEG; Astro creates responsive derivatives
  - id: altar-live
    image: ../../assets/heavy-kept-changing-shape-6.jpg
    alt: Boris and Sunn O))) performing Altar together at ATP New York beneath intense red stage light.
    caption: Boris and Sunn O))) performing the collaborative work Altar at ATP New York, where accumulated tone became an ensemble material.
    creator: Amy Hope Dermont
    sourceName: Wikimedia Commons
    sourceUrl: https://commons.wikimedia.org/wiki/File%3ASunn_O)))_%26_Boris_perform_Altar_%40_ATP_NY.jpg
    license: CC BY 2.0
    licenseUrl: https://creativecommons.org/licenses/by/2.0
    alterations: Resized and converted to optimized JPEG; Astro creates responsive derivatives
sources:
  - id: pitchfork-interview
    title: Boris
    publisher: Pitchfork
    url: https://pitchfork.com/features/interview/6838-boris/
    accessedAt: 2026-08-07
  - id: quietus-interview
    title: "'Noise Is Japanese Blues': An Interview With Boris"
    publisher: The Quietus
    url: https://thequietus.com/interviews/boris-interview/
    accessedAt: 2026-08-07
  - id: drumless-shows
    title: Volume Two "Drumless Shows"
    publisher: Boris Bandcamp
    url: https://boris.bandcamp.com/album/volume-two-drumless-shows
    accessedAt: 2026-08-07
  - id: absolutego
    title: Absolutego
    publisher: Boris Bandcamp
    url: https://boris.bandcamp.com/album/absolutego-2
    accessedAt: 2026-08-07
  - id: flood
    title: Boris performing "flood"
    publisher: Boris Bandcamp
    url: https://boris.bandcamp.com/album/boris-performing-flood
    accessedAt: 2026-08-07
  - id: feedbacker
    title: Boris at Last -Feedbacker-
    publisher: Boris Bandcamp
    url: https://boris.bandcamp.com/album/boris-at-last-feedbacker
    accessedAt: 2026-08-07
  - id: pink-reissue
    title: Just announce 20th Anniversary editions of PINK + dronevil
    publisher: Boris
    url: https://borisheavyrocks.com/news/9585/
    accessedAt: 2026-08-07
relatedSlug: all-four-knobs-to-the-right
---

On _Absolutego_, a guitar chord does not simply arrive and leave. It hangs long enough for its edges to fray. Harmonics swell above it; the low end seems to alter the air around the speakers; a strike that first registered as an event becomes an environment. The original 1996 release presented one piece across roughly an hour. That duration was not decoration applied to a metal riff. It changed what a riff could do. The listener had time to hear volume as movement rather than as a fixed setting. [1](#source-pitchfork-interview) [4](#source-absolutego)

Boris have spent three decades revising that proposition. The Tokyo group formed in 1992 and took its name from a Melvins song, but a lineage is not a method. Takeshi, Wata, and Atsuo learned to make records whose differences are structural: a single sustained composition, a suite that behaves like weather, a compact hard-rock album, or a sequence where shoegaze blur collides with punk velocity. Calling all of it “heavy” is accurate only if the word remains unsettled.

The useful question is therefore not which genre Boris belong to. It is what their records make heaviness mean at a given moment. Sometimes it is the pressure of amplitude. Sometimes it is waiting. Sometimes it is a drum entrance withheld until the ear has recalibrated. Sometimes it is the speed with which a bright riff burns through. The band’s catalog does not trace a clean progression from drone to rock or from obscurity to access. Its forms overlap, recur, and acquire new functions.

## A chord long enough to become a room

The scale of _Absolutego_ can make the piece sound immovable in description. In practice, its slowness exposes change. Feedback gathers around a central mass. Atsuo’s percussion marks distances rather than supplying a continuous beat. Wata and Takeshi do not treat distortion as a curtain hiding the notes; fluctuations inside the distortion become the detail. A small adjustment in density matters because the surrounding time is so extended.

That approach made amplification compositional. An amplifier is normally described as equipment that makes an already formed signal louder. Boris use the whole chain—instrument, pedals, speaker cabinet, room, microphones, and returning feedback—as a system that produces events of its own. Atsuo later told Pitchfork that the band might hang microphones from a ceiling or place them around the drums, then work with incidental sounds and mistakes instead of pursuing one repeatable studio signature. [1](#source-pitchfork-interview)

The point is not romantic accident. An uncontrolled sound becomes useful only when players can recognize and shape it. The trio’s patience allows feedback to develop, but their decisions determine whether it opens space, obscures a pulse, or forces a transition. _Absolutego_ established a method in which duration magnifies those decisions. It also established a problem: once an album-length drone had made scale audible, repeating the same scale would reduce the discovery to a format.

## Amplifier worship, not amplifier obedience

The title _Amplifier Worship_ can be read as a manifesto, yet the 1998 album is less doctrinaire than that sounds. Its pieces retain massive low frequencies and extended pacing, but they make room for quiet guitar, irregular grooves, psychedelic drift, and vocals that seem to emerge from inside the sound. The album does not abandon the amplifier wall. It cuts doors into it.

This is where the trio format becomes important. Boris can sound enormous without pretending that each player has one stable job. Takeshi’s double-neck instrument moves between bass weight and guitar register. Wata can supply a riff, a melody, a sheet of noise, or the nearly empty interval between them. Atsuo acts as drummer, vocalist, and visible conductor of energy. The roles are legible enough for a listener to feel them change.

In a 2014 interview, the band resisted the idea that its many styles were separate costumes. The discussion ranged across collaborations, noise, feedback, and the physical experience of performance; the recurring emphasis was connection rather than genre tourism. [2](#source-quietus-interview) That claim is most convincing in the records themselves. The quiet passages on _Amplifier Worship_ do not apologize for the loud ones. They establish the sensitivity required to hear loudness return differently.

## Flood measures the rise

_Flood_, recorded around the turn of the millennium and first issued in 2000, turns contrast into a four-part arc. Its opening guitar figure is lucid and circular. Repetition does not pin it in place; minute variations make the figure feel as if it is reflecting different light. A fuller band gradually appears, withdraws, and leaves behind a broad suspended field. The final movement brings weight back with enough force to make the earlier restraint part of the impact. [5](#source-flood)

The album’s title invites a simple image of accumulation, but its pacing matters more than the metaphor. A flood is not represented by continuous maximum volume. Boris make the listener register levels: sparse notes, flowing pulse, saturation, recession. Heavy music often advertises impact at the first available second. _Flood_ asks impact to remember what preceded it.

The group’s archive complicates any assumption that drums are indispensable to this architecture. In 2020 Boris released recordings from two 1997 “drumless shows,” explaining that the performances took place when Atsuo could not play because of illness. Wata and Takeshi performed as a duo, with one concert built around material connected to _Absolutego_. [3](#source-drumless-shows) The document matters because it shows the band adapting its mass rather than simulating the missing member. Sustained tone, silence, and the room had to carry a different distribution of weight.

## Heavy Rocks changes the unit

The orange-covered _Heavy Rocks_ of 2002 makes a sharp turn if the early catalog is heard chronologically. Songs arrive as songs. Riffs move with garage-rock impatience; saxophone and guest parts widen the attack; drums push forward rather than measuring an expanse. The title is blunt and almost generic, as though Boris were testing the phrase found on a record-shop divider.

Yet the album does not reveal the band’s “real” rock identity beneath experimental excess. It changes the unit of construction. Instead of asking how a chord can occupy an hour, the record asks how much color and acceleration can fit inside several minutes. Distortion remains tactile, but it is propelled. Repetition becomes a hook or a launch mechanism. The heavy quality lies partly in momentum.

Boris reused the title _Heavy Rocks_ in 2011 and again in 2022. The records are not remakes, and the repeated name refuses the normal archival convenience by which one title identifies one definitive object. It operates more like a recurring workshop label. Each version can examine contemporary hard rock without promising the same songs or production. That practice makes the catalog mildly difficult to sort and exact to its subject: “heavy rocks” is material, not a settled style.

## Feedbacker draws one line through several states

_Boris at Last -Feedbacker-_ returned in 2003 to an album-length composition, but it did not return to the same hour. Often indexed in separate parts, the work behaves as one sequence: a clean, slowly repeated guitar phrase; a widening halo; a crushing central riff; an eruption of feedback; and a final melodic passage that seems marked by everything it has crossed. [6](#source-feedbacker)

The piece makes continuity more dramatic than consistency. Its sections do not merely alternate soft and loud. Sounds retain consequences. The clean motif teaches the ear a contour that can survive distortion. The central riff feels immense because the opening withheld that kind of rhythmic certainty. The feedback passage is not an interlude placed between compositions; it erodes the composition’s surface until another shape can be heard.

This way of building also resists a common shorthand about drone: that it suspends time. Boris make sustained sound reveal time. A held frequency beats against another frequency. Amplifier noise shifts. The body notices fatigue and then a second attention beyond fatigue. On _Feedbacker_, duration is neither an endurance stunt nor an ambient wash. It is the medium through which one state becomes another.

## Pink makes collision audible

_Pink_, first released in Japan in 2005 and issued internationally by Southern Lord in 2006, brought Boris to a wider rock audience. Its accessibility is real but easy to misdescribe. The record does not simplify the band into concise songs. It puts previously separated kinds of force next to one another: charging rock, blown-out punk, bright melody, shoegaze density, and the long dissolution of “Just Abandoned Myself.” The sequence makes speed and suspension mutually explanatory.

The title track compresses the band’s amplifier knowledge into a rapid form. “Farewell” lets melody float inside a luminous, overdriven field. “Just Abandoned Myself” begins with song form and eventually remains inside a vast sustained tail. The album can offer immediate surfaces without treating immediacy as the opposite of depth. Its last section ensures that a listener who arrived for short impact still encounters altered time.

Touring after _Pink_ intensified the relation between record and performance. Atsuo described the cycle as physically exhausting and creatively productive: shows, travel, and recording ran into one another, while each audience created a one-time situation. [1](#source-pitchfork-interview) The remark guards against hearing the albums as sealed studio theories. Boris’s forms are tested against rooms, bodies, equipment, and the practical duration of a set.

The twentieth-anniversary edition of _Pink_ makes its historical position visible without turning it into a conclusion. In 2025 Boris announced a Relapse reissue with previously unreleased session and live material. [7](#source-pink-reissue) An anniversary can freeze an album as a masterpiece. Boris’s subsequent catalog makes that difficult. They continued issuing pop-facing work, noise records, collaborations, ambient drift, crust-punk compression, and further versions of heavy rock.

## No final form

Prolific catalogs encourage two weak responses. One is to celebrate variety as a virtue by itself. The other is to select a small canon and treat everything outside it as detour. Boris reward a more specific account. Their strongest changes retain technical knowledge. The ear trained by _Absolutego_ to notice variation inside saturation can hear how _Pink_ compacts saturation. The dynamic arc of _Flood_ clarifies the transitions inside _Feedbacker_. The drive of _Heavy Rocks_ reveals that repetition can produce velocity as well as stasis.

There are limits to this flexibility. Not every experiment resolves, and abundance can obscure chronology as readily as it enriches it. Different editions, labels, mixes, and identically titled albums make the catalog resistant to a single authoritative map. That resistance should not be inflated into mystique. It is partly the consequence of a working band recording often, collaborating widely, and allowing a release to represent a moment rather than a permanent identity.

What persists is a discipline of listening to material. Volume is not valuable because it is extreme; it is valuable when it exposes texture or reorganizes the room. Duration is not profound because it is long; it matters when a change needs time to register. A fast riff is not a retreat from experiment if speed reveals a different property of the same distorted sound. Boris kept changing shape because each form placed a new demand on the word “heavy.” The catalog does not answer that demand once. It makes the answer audible, lets it decay, and begins again.
