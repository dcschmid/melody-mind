---
title: From Soul to Modern Dance Music
description: >-
  Test your knowledge of how gospel-born soul moved into funk, disco, house, and techno
  through key artists, studios, clubs, and production ideas.
seoTitle: Soul to Dance Music Evolution Quiz | MelodyMind
seoDescription: >-
  Take a soul and dance music quiz covering funk, disco, house, techno, and electronic
  dance music pioneers.
featuredTopics:
  - Soul Music History
  - Funk Evolution
  - House & Techno
category: genre-evolution
questions:
  - id: question-01
    question: >-
      Which artist recorded 'What'd I Say' in 1959, taking the call-and-response structure
      of church into a secular setting?
    type: single-choice
    difficulty: easy
    options:
      - Sam Cooke
      - Ray Charles
      - Otis Redding
      - James Brown
    correct: 1
    explanation: >-
      Ray Charles recorded 'What'd I Say' in 1959, taking the call-and-response structure
      of church and using it in a secular setting full of desire. Some radio stations
      banned the song.
    context: >-
      Ray Charles gives this part of the history a specific person, work, place, or tool.
      Soul joined gospel-derived singing to secular songs, studio bands, independent
      labels, radio, and the demands of the singles market. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Ray Charles
        url: https://en.wikipedia.org/wiki/Ray_Charles
        checkedAt: "2026-07-24"
  - id: question-02
    question: >-
      Which Sam Cooke song, released in 1964 during the Civil Rights Movement, conveyed
      dignity instead of anger?
    type: single-choice
    difficulty: medium
    options:
      - You Send Me
      - A Change Is Gonna Come
      - Wonderful World
      - Cupid
    correct: 1
    explanation: >-
      Sam Cooke's 'A Change Is Gonna Come' was released in 1964 during the Civil Rights
      Movement. It conveyed dignity instead of anger. Cooke died in December 1964, shortly
      after the song's release.
    context: >-
      A Change Is Gonna Come gives this part of the history a specific person, work,
      place, or tool. Soul joined gospel-derived singing to secular songs, studio bands,
      independent labels, radio, and the demands of the singles market. Reading the credit
      or date beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: A Change Is Gonna Come
        url: https://en.wikipedia.org/wiki/A_Change_Is_Gonna_Come
        checkedAt: "2026-07-24"
  - id: question-03
    question: >-
      What was the name of Motown's house band that later became known for creating a
      smooth, radio-ready sound?
    type: single-choice
    difficulty: medium
    options:
      - Booker T. & the M.G.'s
      - The Funk Brothers
      - MFSB
      - The Wrecking Crew
    correct: 1
    explanation: >-
      The band that played in the house at Motown later became known as the Funk Brothers.
      They created a sound that was smooth, full of melody, and perfect for the radio.
    context: >-
      The Funk Brothers gives this part of the history a specific person, work, place, or
      tool. Soul joined gospel-derived singing to secular songs, studio bands, independent
      labels, radio, and the demands of the singles market. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: The Brothers Johnson
        url: https://en.wikipedia.org/wiki/The_Brothers_Johnson
        checkedAt: "2026-07-24"
  - id: question-04
    question: >-
      Which Stax artist's voice moved from soft restraint to explosive power in 'Try a
      Little Tenderness'?
    type: single-choice
    difficulty: medium
    options:
      - Sam Cooke
      - Otis Redding
      - Wilson Pickett
      - Isaac Hayes
    correct: 1
    explanation: >-
      Otis Redding's voice moved from soft restraint to explosive power in 'Try a Little
      Tenderness.' You can hear breathing, strain, and urgency. Stax didn't try to soften
      that intensity.
    context: >-
      Otis Redding gives this part of the history a specific person, work, place, or tool.
      Soul joined gospel-derived singing to secular songs, studio bands, independent
      labels, radio, and the demands of the singles market. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Otis Redding
        url: https://en.wikipedia.org/wiki/Otis_Redding
        checkedAt: "2026-07-24"
  - id: question-05
    question: >-
      Which Marvin Gaye album from 1971 unfolded like a conversation, with songs flowing
      seamlessly into one another?
    type: single-choice
    difficulty: easy
    options:
      - Let's Get It On
      - What's Going On
      - I Want You
      - Trouble Man
    correct: 1
    explanation: >-
      Marvin Gaye released What's Going On in 1971. Motown's reputation was built on tight
      singles, but this album unfolded like a conversation. The songs flowed seamlessly
      into one another.
    context: >-
      What's Going On gives this part of the history a specific person, work, place, or
      tool. Soul joined gospel-derived singing to secular songs, studio bands, independent
      labels, radio, and the demands of the singles market. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: What's Going On
        url: https://en.wikipedia.org/wiki/What's_Going_On
        checkedAt: "2026-07-24"
  - id: question-06
    question: >-
      Which Curtis Mayfield soundtrack examined social conditions rather than glorifying
      the film's drug-dealer protagonist?
    type: single-choice
    difficulty: medium
    options:
      - Shaft
      - Super Fly
      - Foxy Brown
      - The Mack
    correct: 1
    explanation: >-
      Curtis Mayfield's music for the 1972 film Super Fly did not glorify the film's
      drug-dealer protagonist. Instead, Mayfield's songs examined the social conditions of
      that time.
    context: >-
      Super Fly gives this part of the history a specific person, work, place, or tool.
      Soul joined gospel-derived singing to secular songs, studio bands, independent
      labels, radio, and the demands of the singles market. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Super Fly (soundtrack)
        url: https://en.wikipedia.org/wiki/Super_Fly_(soundtrack)
        checkedAt: "2026-07-24"
  - id: question-07
    question: >-
      Which songwriting and production team shaped the Philadelphia soul sound with
      elegance and drive?
    type: single-choice
    difficulty: hard
    options:
      - Holland-Dozier-Holland
      - Kenny Gamble and Leon Huff
      - Nile Rodgers and Bernard Edwards
      - Jimmy Jam and Terry Lewis
    correct: 1
    explanation: >-
      Songwriting and production teams such as Kenny Gamble and Leon Huff worked with a
      stable of musicians who understood how to balance elegance and drive in Philadelphia
      soul.
    context: >-
      Kenny Gamble and Leon Huff gives this part of the history a specific person, work,
      place, or tool. Soul joined gospel-derived singing to secular songs, studio bands,
      independent labels, radio, and the demands of the singles market. Reading the credit
      or date beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Gamble and Huff
        url: https://en.wikipedia.org/wiki/Gamble_and_Huff
        checkedAt: "2026-07-24"
  - id: question-08
    question: >-
      What rhythmic concept did James Brown emphasize as 'the one' - the first beat of
      each measure?
    type: single-choice
    difficulty: medium
    options:
      - Off-beat syncopation
      - The downbeat
      - Polyrhythmic layering
      - Swing timing
    correct: 1
    explanation: >-
      Songs like 'Papa's Got a Brand New Bag' and 'Get Up (I Feel Like Being a) Sex
      Machine' focused a lot on 'the one,' which is the first beat of each measure,
      according to James Brown.
    context: >-
      The downbeat gives this part of the history a specific person, work, place, or tool.
      Soul joined gospel-derived singing to secular songs, studio bands, independent
      labels, radio, and the demands of the singles market.
    sources:
      - publisher: Wikipedia
        title: DownBeat
        url: https://en.wikipedia.org/wiki/DownBeat
        checkedAt: "2026-07-24"
      - publisher: Wikipedia
        title: James Brown
        url: https://en.wikipedia.org/wiki/James_Brown
        checkedAt: "2026-07-24"
  - id: question-09
    question: >-
      Which Sly and the Family Stone album from 1971 sounded darker and more fragmented,
      using a primitive drum machine?
    type: single-choice
    difficulty: hard
    options:
      - Dance to the Music
      - Stand!
      - There's a Riot Goin' On
      - Fresh
    correct: 2
    explanation: >-
      By 1971, Sly and the Family Stone's album There's a Riot Goin' On sounded darker and
      more fragmented. A primitive drum machine, the Maestro Rhythm King MRK-2, was used
      in the production.
    context: >-
      There's a Riot Goin' On gives this part of the history a specific person, work,
      place, or tool. Soul joined gospel-derived singing to secular songs, studio bands,
      independent labels, radio, and the demands of the singles market. Reading the credit
      or date beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: There's a Riot Goin' On
        url: https://en.wikipedia.org/wiki/There's_a_Riot_Goin'_On
        checkedAt: "2026-07-24"
  - id: question-10
    question: >-
      Which 1975 Parliament album made funk music a form of liberation with spaceships and
      characters like Dr. Funkenstein?
    type: single-choice
    difficulty: medium
    options:
      - Funkentelechy Vs. the Placebo Syndrome
      - Mothership Connection
      - The Clones of Dr. Funkenstein
      - Motor Booty Affair
    correct: 1
    explanation: >-
      In 1975, the album Mothership Connection made funk music a form of liberation. The
      images were striking. Spaceships landed on stage. Characters like Dr. Funkenstein
      appeared in the lyrics.
    context: >-
      Mothership Connection gives this part of the history a specific person, work, place,
      or tool. Soul joined gospel-derived singing to secular songs, studio bands,
      independent labels, radio, and the demands of the singles market. Reading the credit
      or date beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Mothership Connection
        url: https://en.wikipedia.org/wiki/Mothership_Connection
        checkedAt: "2026-07-24"
  - id: question-11
    question: >-
      True or False: Chaka Khan was the lead singer of Rufus before launching her solo
      career.
    type: true-false
    difficulty: easy
    options:
      - "True"
      - "False"
    correct: true
    explanation: >-
      True. Chaka Khan was the lead singer of Rufus before launching her solo career.
      'Tell Me Something Good,' written by Stevie Wonder, showed how easily she could move
      from warmth to force in a single phrase.
    context: >-
      The true answer fixes the chronology before later styles are compared with it. Funk
      moved rhythmic weight toward the groove, giving bass, drums, guitar, horns, and
      repeated figures tightly defined roles. Reading the credit or date beside the
      musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Chaka Khan
        url: https://en.wikipedia.org/wiki/Chaka_Khan
        checkedAt: "2026-07-24"
  - id: question-12
    question: >-
      Which Nigerian musician combined highlife, jazz, and American funk to create
      Afrobeat with long, layered rhythms?
    type: single-choice
    difficulty: easy
    options:
      - King Sunny Ade
      - Fela Kuti
      - Miriam Makeba
      - Hugh Masekela
    correct: 1
    explanation: >-
      Fela Kuti combined highlife, jazz, and American funk to create Afrobeat, a style
      with long, layered rhythms and strong political messages. His music was not a copy
      of funk. It expanded its logic.
    context: >-
      Fela Kuti gives this part of the history a specific person, work, place, or tool.
      Funk moved rhythmic weight toward the groove, giving bass, drums, guitar, horns, and
      repeated figures tightly defined roles. Reading the credit or date beside the
      musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Fela Kuti discography
        url: https://en.wikipedia.org/wiki/Fela_Kuti_discography
        checkedAt: "2026-07-24"
  - id: question-13
    question: >-
      Which Jamaican producer was known for breaking tracks down into separate parts,
      removing vocals, and adding echo and reverb?
    type: single-choice
    difficulty: medium
    options:
      - Lee 'Scratch' Perry
      - King Tubby
      - Prince Buster
      - Coxsone Dodd
    correct: 1
    explanation: >-
      King Tubby broke songs down into separate parts, pulling vocals away and bringing
      drums and bass to the front. Echo and reverb became central tools, turning the mix
      itself into a creative space.
    context: >-
      King Tubby gives this part of the history a specific person, work, place, or tool.
      Funk moved rhythmic weight toward the groove, giving bass, drums, guitar, horns, and
      repeated figures tightly defined roles. Reading the credit or date beside the
      musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: King Tubby
        url: https://en.wikipedia.org/wiki/King_Tubby
        checkedAt: "2026-07-24"
  - id: question-14
    question: >-
      What was the name of Lee 'Scratch' Perry's studio where he pushed dub into stranger
      territory?
    type: single-choice
    difficulty: hard
    options:
      - Studio One
      - Black Ark Studio
      - Treasure Isle
      - Tuff Gong
    correct: 1
    explanation: >-
      Lee 'Scratch' Perry worked mainly at his Black Ark Studio in Kingston during the
      1970s, where he pushed dub into stranger territory with dreamlike and sometimes
      chaotic layering.
    context: >-
      Black Ark Studio gives this part of the history a specific person, work, place, or
      tool. Funk moved rhythmic weight toward the groove, giving bass, drums, guitar,
      horns, and repeated figures tightly defined roles. Reading the credit or date beside
      the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Black Ark Studios
        url: https://en.wikipedia.org/wiki/Black_Ark_Studios
        checkedAt: "2026-07-24"
  - id: question-15
    question: >-
      Which British producer used his On-U Sound label to mix dub with post-punk and early
      electronic experimentation?
    type: single-choice
    difficulty: hard
    options:
      - Adrian Sherwood
      - John Peel
      - Dennis Bovell
      - Jah Shaka
    correct: 0
    explanation: >-
      In the late 1970s and 1980s, Adrian Sherwood used his On-U Sound label to mix dub
      music with post-punk and early electronic experimentation.
    context: >-
      Adrian Sherwood gives this part of the history a specific person, work, place, or
      tool. Funk moved rhythmic weight toward the groove, giving bass, drums, guitar,
      horns, and repeated figures tightly defined roles. Reading the credit or date beside
      the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Adrian Sherwood
        url: https://en.wikipedia.org/wiki/Adrian_Sherwood
        checkedAt: "2026-07-24"
  - id: question-16
    question: >-
      Which DJ approached DJing as a form of storytelling at Paradise Garage, building
      tension slowly with extended breaks?
    type: single-choice
    difficulty: medium
    options:
      - David Mancuso
      - Larry Levan
      - Frankie Knuckles
      - Ron Hardy
    correct: 1
    explanation: >-
      Larry Levan approached DJing as a form of storytelling at Paradise Garage. He would
      build tension slowly, sometimes letting a record loop through extended breaks,
      sometimes dropping in unexpected selections.
    context: >-
      Larry Levan gives this part of the history a specific person, work, place, or tool.
      Funk moved rhythmic weight toward the groove, giving bass, drums, guitar, horns, and
      repeated figures tightly defined roles. Reading the credit or date beside the
      musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Larry Levan
        url: https://en.wikipedia.org/wiki/Larry_Levan
        checkedAt: "2026-07-24"
  - id: question-17
    question: >-
      What innovation allowed records to be pressed on larger vinyl with wider grooves for
      better sound quality and longer play time?
    type: single-choice
    difficulty: medium
    options:
      - The 7-inch single
      - The 12-inch single
      - The extended play (EP)
      - The quadrophonic disc
    correct: 1
    explanation: >-
      The 12-inch single was pressed on larger vinyl with wider grooves, which allowed for
      better sound quality and longer play time. The bass sounds fuller. Drums carried
      more weight.
    context: >-
      The 12-inch single gives this part of the history a specific person, work, place, or
      tool. Funk moved rhythmic weight toward the groove, giving bass, drums, guitar,
      horns, and repeated figures tightly defined roles. Reading the credit or date beside
      the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Twelve-inch single
        url: https://en.wikipedia.org/wiki/Twelve-inch_single
        checkedAt: "2026-07-24"
  - id: question-18
    question: >-
      Which 1977 Donna Summer track used a pulsing synthesizer pattern without a full
      orchestra or live band?
    type: single-choice
    difficulty: easy
    options:
      - Love to Love You Baby
      - I Feel Love
      - Hot Stuff
      - Bad Girls
    correct: 1
    explanation: >-
      'I Feel Love' from 1977 replaced the lush orchestral approach heard in much disco
      with an insistent sequenced synthesizer pattern. Giorgio Moroder and his
      collaborators built the track around a Moog-driven pulse, making it sound futuristic
      and helping point dance music toward a more fully electronic future.
    context: >-
      I Feel Love gives this part of the history a specific person, work, place, or tool.
      Funk moved rhythmic weight toward the groove, giving bass, drums, guitar, horns, and
      repeated figures tightly defined roles.
    sources:
      - publisher: Wikipedia
        title: I Feel Love
        url: https://en.wikipedia.org/wiki/I_Feel_Love
        checkedAt: "2026-07-24"
  - id: question-19
    question: >-
      Who was the Italian producer that worked closely with Donna Summer on electronic
      disco productions?
    type: single-choice
    difficulty: medium
    options:
      - Nile Rodgers
      - Giorgio Moroder
      - Pete Bellotte
      - Harold Faltermeyer
    correct: 1
    explanation: >-
      In Munich, Italian producer Giorgio Moroder started experimenting with synthesizers.
      He worked closely with Donna Summer and combined disco's steady beat with electronic
      sequencing.
    context: >-
      Giorgio Moroder gives this part of the history a specific person, work, place, or
      tool. Funk moved rhythmic weight toward the groove, giving bass, drums, guitar,
      horns, and repeated figures tightly defined roles. Reading the credit or date beside
      the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Giorgio Moroder
        url: https://en.wikipedia.org/wiki/Giorgio_Moroder
        checkedAt: "2026-07-24"
  - id: question-20
    question: >-
      Which Chic founding members approached disco with discipline and clarity, building
      records on precision rather than excess?
    type: multi-choice
    difficulty: medium
    options:
      - Nile Rodgers
      - Bernard Edwards
      - Tony Thompson
      - Norma Jean Wright
    correct:
      - 0
      - 1
    explanation: >-
      Guitarist Nile Rodgers and bassist Bernard Edwards founded Chic, a band that
      approached disco with discipline and clarity. Their records were built on precision
      rather than excess.
    context: >-
      Nile Rodgers, Bernard Edwards belong together here; the comparison names each part
      of the change. Funk moved rhythmic weight toward the groove, giving bass, drums,
      guitar, horns, and repeated figures tightly defined roles. Reading the credit or
      date beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Chic (band)
        url: https://en.wikipedia.org/wiki/Chic_(band)
        checkedAt: "2026-07-24"
  - id: question-21
    question: >-
      Which disco event in July 1979 at Chicago's Comiskey Park involved destroying disco
      records between baseball games?
    type: single-choice
    difficulty: medium
    options:
      - The Disco Burn
      - Disco Demolition Night
      - The Anti-Disco Rally
      - Rock Against Disco
    correct: 1
    explanation: >-
      Disco Demolition Night at Chicago's Comiskey Park in July 1979 invited fans to bring
      disco records to be destroyed between baseball games. The event quickly spun out of
      control as crowds rushed the field and records were burned and smashed.
    context: >-
      Disco Demolition Night gives this part of the history a specific person, work,
      place, or tool. Disco records were arranged for dancers and extended club play,
      which changed song length, mixing, orchestration, and the work of DJs.
    sources:
      - publisher: Wikipedia
        title: July 16
        url: https://en.wikipedia.org/wiki/July_16
        checkedAt: "2026-07-24"
  - id: question-22
    question: >-
      Which 1982 Afrika Bambaataa track was inspired by European electronic music,
      especially Kraftwerk?
    type: single-choice
    difficulty: medium
    options:
      - Looking for the Perfect Beat
      - Planet Rock
      - Renegades of Funk
      - Rockit
    correct: 1
    explanation: >-
      In 1982, Afrika Bambaataa released 'Planet Rock,' which he produced with Arthur
      Baker and John Robie. The track was inspired by European electronic music,
      especially Kraftwerk, but also had a strong drum machine beat.
    context: >-
      Planet Rock gives this part of the history a specific person, work, place, or tool.
      Disco records were arranged for dancers and extended club play, which changed song
      length, mixing, orchestration, and the work of DJs.
    sources:
      - publisher: Wikipedia
        title: Planet Rock
        url: https://en.wikipedia.org/wiki/Planet_Rock
        checkedAt: "2026-07-24"
  - id: question-23
    question: >-
      Which drum machine was used to create the clear, strong rhythm in Afrika Bambaataa's
      'Planet Rock'?
    type: single-choice
    difficulty: hard
    options:
      - Roland TR-909
      - Roland TR-808
      - LinnDrum
      - Oberheim DMX
    correct: 1
    explanation: >-
      The Roland TR-808 drum machine produced a clear, strong rhythm in 'Planet Rock.' The
      bass moved in repeating electronic patterns. The vocals were more like a chant than
      a typical soul performance.
    context: >-
      Roland TR-808 gives this part of the history a specific person, work, place, or
      tool. Disco records were arranged for dancers and extended club play, which changed
      song length, mixing, orchestration, and the work of DJs. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Roland TR-808
        url: https://en.wikipedia.org/wiki/Roland_TR-808
        checkedAt: "2026-07-24"
  - id: question-24
    question: >-
      Which style emerged in New York's Latin club scene, mixing electronic drum
      programming with melodies inspired by disco and R&B?
    type: single-choice
    difficulty: medium
    options:
      - Electro
      - Freestyle
      - Hi-NRG
      - Boogie
    correct: 1
    explanation: >-
      Freestyle emerged in New York's Latin club scene. It mixed electronic drum
      programming with melodies that were inspired by disco and R&B. Artists like Lisa
      Lisa and Shannon helped create the distinctive sound.
    context: >-
      Freestyle gives this part of the history a specific person, work, place, or tool.
      Disco records were arranged for dancers and extended club play, which changed song
      length, mixing, orchestration, and the work of DJs. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Freestyle rap
        url: https://en.wikipedia.org/wiki/Freestyle_rap
        checkedAt: "2026-07-24"
  - id: question-25
    question: >-
      Which Chicago DJ worked at the Warehouse and is often called the 'Godfather of
      House'?
    type: single-choice
    difficulty: easy
    options:
      - Frankie Knuckles
      - Ron Hardy
      - Marshall Jefferson
      - Larry Heard
    correct: 0
    explanation: >-
      Frankie Knuckles was a central figure in the house music shift. He worked at a club
      called the Warehouse, where he created a unique sound that felt familiar but
      simpler. The term 'house' likely came from the warehouse itself.
    context: >-
      Frankie Knuckles gives this part of the history a specific person, work, place, or
      tool. Disco records were arranged for dancers and extended club play, which changed
      song length, mixing, orchestration, and the work of DJs.
    sources:
      - publisher: Wikipedia
        title: Frankie Knuckles
        url: https://en.wikipedia.org/wiki/Frankie_Knuckles
        checkedAt: "2026-07-24"
  - id: question-26
    question: >-
      Which bass synthesizer would later define acid house and was associated with Ron
      Hardy's sets at the Music Box?
    type: single-choice
    difficulty: hard
    options:
      - Roland TR-808
      - Roland TB-303
      - Korg MS-20
      - Moog Minimoog
    correct: 1
    explanation: >-
      One of the sounds associated with Ron Hardy's sets was the Roland TB-303 bass
      synthesizer, which would later define acid house. The bass lines of early acid
      tracks sounded rough compared to polished disco.
    context: >-
      Roland TB-303 gives this part of the history a specific person, work, place, or
      tool. Disco records were arranged for dancers and extended club play, which changed
      song length, mixing, orchestration, and the work of DJs.
    sources:
      - publisher: Wikipedia
        title: "Synthesizing: Ten Ragas to a Disco Beat"
        url: https://en.wikipedia.org/wiki/Synthesizing%3A_Ten_Ragas_to_a_Disco_Beat
        checkedAt: "2026-07-24"
  - id: question-27
    question: >-
      Which 1984 Jesse Saunders track is considered one of the first house records to
      become popular?
    type: single-choice
    difficulty: hard
    options:
      - Move Your Body
      - On and On
      - Can You Feel It
      - Your Love
    correct: 1
    explanation: >-
      One of the first house records to become popular was 'On and On' by Jesse Saunders,
      released in 1984. The song was built around a drum machine pattern and a repeating
      bass line. It was simple compared to most disco productions.
    context: >-
      On and On gives this part of the history a specific person, work, place, or tool.
      Disco records were arranged for dancers and extended club play, which changed song
      length, mixing, orchestration, and the work of DJs.
    sources:
      - publisher: Wikipedia
        title: Jesse Saunders
        url: https://en.wikipedia.org/wiki/Jesse_Saunders
        checkedAt: "2026-07-24"
      - publisher: Wikipedia
        title: On & On (Jesse Saunders song)
        url: https://en.wikipedia.org/wiki/On_%26_On_(Jesse_Saunders_song)
        checkedAt: "2026-07-24"
  - id: question-28
    question: >-
      Which Marshall Jefferson track from 1986 used piano notes and gospel-influenced
      chords, becoming known as a 'house anthem'?
    type: single-choice
    difficulty: medium
    options:
      - Move Your Body
      - On and On
      - Mystery of Love
      - Open Our Eyes
    correct: 0
    explanation: >-
      Marshall Jefferson's 'Move Your Body' from 1986 used piano notes and happy chords
      that reminded people of gospel-influenced disco music. The record quickly became
      known as a 'house anthem.'
    context: >-
      Move Your Body gives this part of the history a specific person, work, place, or
      tool. Disco records were arranged for dancers and extended club play, which changed
      song length, mixing, orchestration, and the work of DJs. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Move Your Body
        url: https://en.wikipedia.org/wiki/Move_Your_Body
        checkedAt: "2026-07-24"
  - id: question-29
    question: Which independent record label was key in spreading early Chicago house tracks?
    type: single-choice
    difficulty: hard
    options:
      - Trax Records
      - DJ International
      - Strictly Rhythm
      - Defected
    correct: 0
    explanation: >-
      Independent record labels like Trax Records were key in spreading these early
      tracks. Sometimes, there weren't many records available. The quality control was
      inconsistent. But the records spread quickly through DJ networks.
    context: >-
      Trax Records gives this part of the history a specific person, work, place, or tool.
      Disco records were arranged for dancers and extended club play, which changed song
      length, mixing, orchestration, and the work of DJs. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Trax Records
        url: https://en.wikipedia.org/wiki/Trax_Records
        checkedAt: "2026-07-24"
  - id: question-30
    question: >-
      Which 1987 CeCe Rogers track produced by Marshall Jefferson had a clear spiritual
      tone and became a vocal house classic?
    type: single-choice
    difficulty: hard
    options:
      - Someday
      - Got to Get You Back
      - All Join Hands
      - CeCe's Pen
    correct: 0
    explanation: >-
      Tracks like 'Someday' by CeCe Rogers, produced by Marshall Jefferson in 1987, had a
      clear spiritual tone. The lyrics conveyed hope and perseverance. The piano chords
      made the rhythm feel more lively.
    context: >-
      Someday gives this part of the history a specific person, work, place, or tool.
      Disco records were arranged for dancers and extended club play, which changed song
      length, mixing, orchestration, and the work of DJs. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Maybe Someday
        url: https://en.wikipedia.org/wiki/Maybe_Someday
        checkedAt: "2026-07-24"
  - id: question-31
    question: >-
      Which Juan Atkins project released the 1983 track 'Clear' that mixed strong drum
      machine beats with soft synthesizer sounds?
    type: single-choice
    difficulty: hard
    options:
      - Model 500
      - Cybotron
      - Infiniti
      - Channel One
    correct: 1
    explanation: >-
      Juan Atkins released early recordings under the name Cybotron. In 1983, the song
      'Clear' mixed strong drum machine beats with soft synthesizer sounds. The atmosphere
      felt like science fiction but wasn't over-the-top.
    context: >-
      Cybotron gives this part of the history a specific person, work, place, or tool.
      House and techno producers used drum machines, synthesizers, samplers, and clubs to
      build different forms of electronic dance music. Reading the credit or date beside
      the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Cybotron
        url: https://en.wikipedia.org/wiki/Cybotron
        checkedAt: "2026-07-24"
  - id: question-32
    question: >-
      Which Derrick May track from 1987 used energetic chord sequences and a strong piano
      sound, released under the name Rhythim Is Rhythim?
    type: single-choice
    difficulty: hard
    options:
      - Nude Photo
      - Strings of Life
      - The Dance
      - Kao-Tic Harmony
    correct: 1
    explanation: >-
      Derrick May's 1987 track 'Strings of Life,' released under the name Rhythim Is
      Rhythim, used a steady electronic beat, energetic chord sequences, and a strong
      piano sound. The record felt urgent and emotional.
    context: >-
      Strings of Life gives this part of the history a specific person, work, place, or
      tool. House and techno producers used drum machines, synthesizers, samplers, and
      clubs to build different forms of electronic dance music. Reading the credit or date
      beside the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Strings of Life
        url: https://en.wikipedia.org/wiki/Strings_of_Life
        checkedAt: "2026-07-24"
  - id: question-33
    question: >-
      Which Kevin Saunderson project produced 'Big Fun' and 'Good Life,' balancing
      techno's pulse with catchy vocal hooks?
    type: single-choice
    difficulty: medium
    options:
      - Inner City
      - Reese
      - E-Dancer
      - EssRay
    correct: 0
    explanation: >-
      Kevin Saunderson produced 'Big Fun' and 'Good Life' under the name Inner City. These
      tracks balanced techno's mechanical pulse with catchy vocal hooks. These records
      were successful in the United Kingdom.
    context: >-
      Inner City gives this part of the history a specific person, work, place, or tool.
      House and techno producers used drum machines, synthesizers, samplers, and clubs to
      build different forms of electronic dance music. Reading the credit or date beside
      the musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Inner City Life
        url: https://en.wikipedia.org/wiki/Inner_City_Life
        checkedAt: "2026-07-24"
  - id: question-34
    question: >-
      Which Detroit group was founded by Jeff Mills, 'Mad' Mike Banks, and Robert Hood,
      rejecting commercial gloss and emphasizing autonomy?
    type: single-choice
    difficulty: medium
    options:
      - Underground Resistance
      - Drexciya
      - Aux 88
      - Model 500
    correct: 0
    explanation: >-
      In the late 1980s, Underground Resistance became a strong voice for the Detroit
      music scene. The group was founded by Jeff Mills, 'Mad' Mike Banks, and Robert Hood.
      They rejected commercial gloss and emphasized autonomy.
    context: >-
      Underground Resistance gives this part of the history a specific person, work,
      place, or tool. House and techno producers used drum machines, synthesizers,
      samplers, and clubs to build different forms of electronic dance music. Reading the
      credit or date beside the musical detail makes the answer easier to check and
      compare.
    sources:
      - publisher: Wikipedia
        title: Underground Resistance
        url: https://en.wikipedia.org/wiki/Underground_Resistance
        checkedAt: "2026-07-24"
  - id: question-35
    question: >-
      Which Berlin club opened in 1991 in the basement of a former department store and
      became central to techno culture?
    type: single-choice
    difficulty: medium
    options:
      - Berghain
      - Tresor
      - Watergate
      - Kitkat Club
    correct: 1
    explanation: >-
      Clubs like Tresor, which opened in 1991 in the basement of a former department
      store, became central meeting points. Detroit records were imported and played
      loudly. The simple, intense style was especially appealing.
    context: >-
      Tresor gives this part of the history a specific person, work, place, or tool. House
      and techno producers used drum machines, synthesizers, samplers, and clubs to build
      different forms of electronic dance music. Reading the credit or date beside the
      musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Tresor (club)
        url: https://en.wikipedia.org/wiki/Tresor_(club)
        checkedAt: "2026-07-24"
  - id: question-36
    question: >-
      Which New York duo continued disco's emotional tradition in the 1990s, mixing house
      with Latin percussion and jazz?
    type: single-choice
    difficulty: medium
    options:
      - Masters at Work (Louie Vega and Kenny 'Dope' Gonzalez)
      - Basement Jaxx
      - Deep Dish
      - Dimitri from Paris
    correct: 0
    explanation: >-
      The duo Masters at Work, made up of Louie Vega and Kenny 'Dope' Gonzalez, continued
      the garage tradition in the 1990s. Their music mixed house music with Latin
      percussion, jazz, and strong singing.
    context: >-
      Masters at Work (Louie Vega and Kenny 'Dope' Gonzalez) gives this part of the
      history a specific person, work, place, or tool. House and techno producers used
      drum machines, synthesizers, samplers, and clubs to build different forms of
      electronic dance music.
    sources:
      - publisher: Wikipedia
        title: Masters at Work discography
        url: https://en.wikipedia.org/wiki/Masters_at_Work_discography
        checkedAt: "2026-07-24"
  - id: question-37
    question: >-
      Which French electronic duo's 1997 album Homework was mostly raw house and techno,
      while 2001's Discovery was more openly melodic?
    type: single-choice
    difficulty: easy
    options:
      - Justice
      - Cassius
      - Daft Punk
      - Air
    correct: 2
    explanation: >-
      Daft Punk's 1997 album Homework was mostly raw house and techno, but by the time of
      Discovery in 2001, the duo was more open about the melodic richness of disco. 'One
      More Time' had processed vocals and bright, looping chords.
    context: >-
      Daft Punk gives this part of the history a specific person, work, place, or tool.
      House and techno producers used drum machines, synthesizers, samplers, and clubs to
      build different forms of electronic dance music.
    sources:
      - publisher: Wikipedia
        title: Daft Punk
        url: https://en.wikipedia.org/wiki/Daft_Punk
        checkedAt: "2026-07-24"
  - id: question-38
    question: >-
      Which South African DJ helped make Afro-house popular at international festivals
      with deep house beats and local percussion?
    type: single-choice
    difficulty: medium
    options:
      - Black Coffee
      - Da Capo
      - Caiiro
      - Manoo
    correct: 0
    explanation: >-
      When Black Coffee gained major international attention in the 2010s, he helped bring
      Afro-house to larger global stages. His tracks often combine deep-house structure
      with local rhythmic detail, restrained melodic writing, and production that feels
      spacious rather than overloaded.
    context: >-
      Black Coffee gives this part of the history a specific person, work, place, or tool.
      House and techno producers used drum machines, synthesizers, samplers, and clubs to
      build different forms of electronic dance music.
    sources:
      - publisher: Wikipedia
        title: Black Coffee (DJ)
        url: https://en.wikipedia.org/wiki/Black_Coffee_(DJ)
        checkedAt: "2026-07-24"
  - id: question-39
    question: >-
      Which Chicago-born trans DJ and producer mixes classic house with modern production
      while proudly declaring her identity?
    type: single-choice
    difficulty: medium
    options:
      - Honey Dijon
      - The Blessed Madonna
      - Nina Kraviz
      - Tokimonsta
    correct: 0
    explanation: >-
      Honey Dijon is an important voice in dance music evolution. She comes from a family
      of musicians in Chicago. Her sets pay tribute to disco, garage, and techno while
      proudly declaring her identity as a trans woman.
    context: >-
      Honey Dijon gives this part of the history a specific person, work, place, or tool.
      House and techno producers used drum machines, synthesizers, samplers, and clubs to
      build different forms of electronic dance music.
    sources:
      - publisher: Wikipedia
        title: Honey Dijon
        url: https://en.wikipedia.org/wiki/Honey_Dijon
        checkedAt: "2026-07-24"
  - id: question-40
    question: >-
      True or False: Digital audio workstations (DAWs) like Ableton Live allowed producers
      to create complete songs on laptops without major label budgets.
    type: true-false
    difficulty: easy
    options:
      - "True"
      - "False"
    correct: true
    explanation: >-
      True. By the 2000s, digital audio workstations (DAWs) allowed producers to create,
      arrange, and mix complete songs on a laptop. Software like Ableton Live and Logic
      Pro brought professional tools into home studios.
    context: >-
      The true answer fixes the chronology before later styles are compared with it. House
      and techno producers used drum machines, synthesizers, samplers, and clubs to build
      different forms of electronic dance music. Reading the credit or date beside the
      musical detail makes the answer easier to check and compare.
    sources:
      - publisher: Wikipedia
        title: Digital audio workstation
        url: https://en.wikipedia.org/wiki/Digital_audio_workstation
        checkedAt: "2026-07-24"
---
