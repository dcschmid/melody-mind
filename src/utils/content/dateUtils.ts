 ...
 */
export function derivePublishModified(publishRaw: unknown, modifiedRaw: unknown): DeriveDatesResult {
  const publish = normalizeDate(publishRaw);
  const modified = normalizeDate(modifiedRaw);
  if (publish && !modified) { return { publishDate: publish, modifiedDate: publish }; }
  if (!publish && modified) { return { publishDate: modified, modifiedDate: modified }; }
