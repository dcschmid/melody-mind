...
import albums from "../../json/albums.json"; // JSON file containing albums data
import { shuffleArray } from "../../utils/share/shuffleArray";
import { filterAlbumsByCategory } from "../../utils/share/filterAlbumsByCategroy";
 ...
 * @param {string} category - The category of albums to filter by.
 * @return {Album[]} A list of sorted albums.
 */
export function getAlbumsSortedBySpecificFieldAndCatgory(sortBy: string, order: string, category: string) {
  // Filter the albums by category
  const randomAlbums = shuffleArray(filterAlbumsByCategory(albums, category)).slice(0, 4);
