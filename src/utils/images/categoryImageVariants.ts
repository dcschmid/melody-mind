...
): { avif: VariantEntry[]; webp: VariantEntry[]; fallback: string | null } {
 ...
 */
export async function getCategoryImageVariants(
  slug: string
): Promise<CategoryImageVariants | null> {
  if (!slug) {
    return null;
  }
  ...
  const candidateEntries = Object.entries(variantLoaders).filter(([p]) =>
