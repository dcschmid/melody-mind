...
export function calculateTheCurrentPoints(category: string) {
  ...
  const pointsRound1 = parseInt(
    localStorage.getItem(`${category}-Points-round-one`) || "0",
    10,
  );
  const pointsRound2 = parseInt(
    localStorage.getItem(`${category}-Points-round-two`) || "0",
    10,
  );
  const pointsRound3 = parseInt(
    localStorage.getItem(`${category}-Points-round-three`) || "0",
    10,
  );
  ...
  const totalPoints =
    calculatedPointsRound1 + calculatedPointsRound2 + calculatedPointsRound3;
