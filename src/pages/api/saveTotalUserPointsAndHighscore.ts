...
import { and, db, eq, HighscorePerCategory, TotalHighscore, User } from "astro:db";
...
export const POST: APIRoute = async ({ request }) => {
   ...
   */
  const { userId, totalUserPoints, category, categoryPoints } = await request.json();
  ...
  const isUserInHighscore = await db.select().from(TotalHighscore)
  ...
  const isUserInHighScoreInCategory = await db.select().from(HighscorePerCategory)
    .where(and(eq(HighscorePerCategory.userId, userId), eq(HighscorePerCategory.category, category)));
  ...
  await db.batch([
    ...
    isUserInHighscore.length > 0 ?
      db.update(TotalHighscore).set({ score: totalUserPoints }).where(eq(TotalHighscore.userId, userId)) :
      db.insert(TotalHighscore).values({id: generateIdFromEntropySize(10), userId, score: totalUserPoints }),
    /**
