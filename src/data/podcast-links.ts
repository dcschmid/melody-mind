...
export const podcastLinks: Record<PodcastLanguage, Record<PodcastPlatform, string>> = {
  de: {
  ...
  en: {
    ...
    deezer: 'https://www.deezer.com/show/3Lxwc4ciAYXAMJdyWUqs45', // Hinweis: Der Link scheint identisch mit Spotify zu sein
 ...
 */
export function getPodcastLink(language: PodcastLanguage, platform: PodcastPlatform): string {
 ...
 */
export function getPodcastLinks(language: PodcastLanguage): Record<PodcastPlatform, string> {
  // Fallback to 'en' if language not available
  const lang = language in podcastLinks ? language : 'en';
