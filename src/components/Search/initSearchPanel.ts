...
export function initSearchPanel(
  ...
  const { idBase, itemSelector, examplesContainerId, exampleButtonSelector, onNoResults, onResultsUpdated, debounceMs } = opts;
  ...
  const autoStatusCountTpl = statusEl?.getAttribute('data-search-status-count') || '{count} results';
  const noResultsTpl = noResultsEl?.getAttribute('data-search-no-results-tpl') || 'No results for "{term}"';

  // Determine effective debounce using heuristic only if not explicitly provided
  const effectiveDebounce = typeof debounceMs === 'number' ? debounceMs : deriveHeuristicDebounce(itemSelector);
