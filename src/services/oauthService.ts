...
import { oauthProviders, generateOAuthState, generateCodeVerifier, generateCodeChallenge, getOAuthRedirectUri } from '../config/oauth';
import type { 
  OAuthProvider, 
  OAuthProviderConfig, 
  OAuthSession, 
...
export class OAuthService {
  ...
  async handleCallback(
    ...
    console.log('OAuth handleCallback called with:', { provider, code: code ? 'present' : 'missing', state, baseUrl });
   ...
   */
  async unlinkProvider(userId: string, provider: OAuthProvider): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  private async getOAuthSession(state: string): Promise<OAuthSession | null> {
    console.log('Getting OAuth session for state:', state);
    ...
    console.log('OAuth session query result:', result.rows);
    ...
    switch (provider) {
      ...
      case 'discord':
        ...
        return {
          ...
          avatarUrl: discordData.avatar ? `https://cdn.discordapp.com/avatars/${discordData.id}/${discordData.avatar}.png` : undefined,
  ...
  private async getUserOAuthProvider(userId: string, provider: OAuthProvider): Promise<OAuthProviderAccount | null> {
    const result = await turso.execute({
  ...
  private async getOAuthProviderByProviderUserId(provider: OAuthProvider, providerUserId: string): Promise<OAuthProviderAccount | null> {
